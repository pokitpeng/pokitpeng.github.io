---
title: "Protobuf初体验"
date: 2020-07-16T11:14:35+08:00
draft: false
tags:
    - RPC
    - golang
...
从 [Protobuf Releases](https://github.com/protocolbuffers/protobuf) 下载最先版本的发布包，解压安装包，并将bin目录添加到系统环境变量。
...
```
大概率会安装失败，原因是内部的一些依赖被墙了，挂代理也有一定几率会失败。无奈~，那就手动编译吧：
```
...
## 定义消息类型
我们创建一个非常简单的示例进行测试，`student.proto`
```proto
syntax = "proto3";
package main;
...
```
将该目录下的所有的`.proto`文件转换为 Go 代码，我们可以看到该目录下多出了一个 Go 文件 `student.pb.go`。这个文件内部定义了一个结构体 Student，以及相关的方法
...
```
- 保留字段(Reserved Field)
更新消息类型时，可能会将某些字段/标识符删除。这些被删掉的字段/标识符可能被重新使用，如果加载老版本的数据时，可能会造成数据冲突，在升级时，可以将这些字段/标识符保留(reserved)，这样就不会被重新使用了，protoc 会检查。例如：
```proto
message Foo {
